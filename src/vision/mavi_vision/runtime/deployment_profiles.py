...
def _load_raw(path: Path) -> dict[str, Any]:
    try:
        ...
        raise DeploymentProfileError(
            "deployment_profile_policy_invalid"
        ) from exc
...
def load_policy(
    ...
    if (
        ...
        != "mavi-phase1-deployment-profiles-v1"
    ):
        raise DeploymentProfileError(
            "deployment_profile_schema_invalid"
        )
    profiles_raw = raw.get("profiles")
    if (
        not isinstance(profiles_raw, dict)
        or set(profiles_raw) != _ALLOWED_PROFILE_IDS
    ):
        raise DeploymentProfileError(
            "deployment_profile_set_invalid"
        )
    ...
    for profile_id in sorted(_ALLOWED_PROFILE_IDS):
        ...
        if not isinstance(item, dict):
            raise DeploymentProfileError(
                "deployment_profile_invalid:" + profile_id
            )
        if item.get("id") != profile_id:
            raise DeploymentProfileError(
                "deployment_profile_id_mismatch:" + profile_id
            )

        name = item.get("name")
        description = item.get("description")
        runtime_variant = item.get("runtimeVariant")
        offline_install_gate = item.get(
            "offlineInstallGate"
        )
        ...
        if not all(
            ...
            raise DeploymentProfileError(
                "deployment_profile_text_invalid:" + profile_id
            )
        if runtime_variant not in _ALLOWED_VARIANTS:
            raise DeploymentProfileError(
                "deployment_profile_variant_invalid:" + profile_id
            )
        if (
            performance_gate is not None
            and not _nonempty_text(performance_gate)
        ):
            raise DeploymentProfileError(
                "deployment_profile_performance_gate_invalid:"
                + profile_id
            )
        if not isinstance(
        ...
        if (
            ...
            or any(
                role not in _ALLOWED_PREREQUISITE_ROLES
                for role in roles
            )
        ):
            raise DeploymentProfileError(
                "deployment_profile_roles_invalid:" + profile_id
            )
        if vision_host_role not in roles:
            raise DeploymentProfileError(
                "deployment_profile_vision_role_invalid:"
                + profile_id
            )
        if not isinstance(requires_cuda, bool):
            raise DeploymentProfileError(
                "deployment_profile_cuda_flag_invalid:"
                + profile_id
            )
        if requires_cuda != runtime_variant.endswith("-cuda"):
            raise DeploymentProfileError(
                "deployment_profile_cuda_variant_mismatch:"
                + profile_id
            )
    ...
    if (
        result["P1"].runtime_variant
        != "windows-x86_64-cuda"
    ):
        raise DeploymentProfileError(
            "deployment_profile_p1_contract_invalid"
        )
    if (
        result["P2"].runtime_variant
        != "linux-x86_64-cuda"
    ):
        raise DeploymentProfileError(
            "deployment_profile_p2_contract_invalid"
        )
    if (
        result["P3"].runtime_variant
        != "windows-x86_64-cpu"
    ):
        raise DeploymentProfileError(
            "deployment_profile_p3_contract_invalid"
        )
...
def required_qualification_gates(
    ...
    if (
        ...
        raise DeploymentProfileError(
            "deployment_profile_selection_invalid"
        )
    gates: set[str] = set()
    for profile_id in selected:
        try:
            gates.update(
                profiles[profile_id].qualification_gates
            )
        except KeyError as exc:
            raise DeploymentProfileError(
                "deployment_profile_unknown:" + profile_id
