...
@pytest.mark.asyncio
async def test_execution_lane_serializes_work_on_one_dedicated_thread() -> None:
    ...
    try:
        ...
        started = await asyncio.to_thread(first_started.wait, 5.0)
...
@pytest.mark.asyncio
async def test_execution_lane_forwards_arguments_and_return_value() -> None:
    lane = VisionExecutionLane()
...
@pytest.mark.asyncio
async def test_execution_lane_propagates_callable_exception() -> None:
    lane = VisionExecutionLane()
...
@pytest.mark.asyncio
async def test_execution_lane_rejects_work_after_close() -> None:
    lane = VisionExecutionLane()
    await lane.close()
    await lane.close()
...
@pytest.mark.asyncio
async def test_execution_lane_cancellation_does_not_cancel_native_work() -> None:
    lane = VisionExecutionLane()
...
@pytest.mark.asyncio
async def test_execution_lane_close_waits_for_already_submitted_work() -> None:
    lane = VisionExecutionLane()
    started = threading.Event()
    release = threading.Event()
    completed = threading.Event()
