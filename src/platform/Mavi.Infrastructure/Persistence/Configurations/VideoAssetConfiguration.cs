...
public sealed class VideoAssetConfiguration : IEntityTypeConfiguration<VideoAsset>
{
    ...
    {
        builder.ToTable("video_assets", table => { table.HasCheckConstraint("ck_video_assets_duration", "duration_ms > 0"); table.HasCheckConstraint("ck_video_assets_dimensions", "width > 0 AND height > 0"); table.HasCheckConstraint("ck_video_assets_frame_rate", "frame_rate_numerator > 0 AND frame_rate_denominator > 0"); table.HasCheckConstraint("ck_video_assets_timestamp_confidence", "timestamp_confidence >= 0 AND timestamp_confidence <= 1"); });
