...
public sealed class VisionJobConfiguration : IEntityTypeConfiguration<VisionJob>
{
    ...
    {
        builder.ToTable("vision_jobs", t => { t.HasCheckConstraint("ck_vision_jobs_attempts", "attempt_count >= 0"); t.HasCheckConstraint("ck_vision_jobs_progress", "progress_percent >= 0 AND progress_percent <= 100"); }); builder.HasKey(x => x.Id);
