...
if (-not (Test-Path -LiteralPath $manifestPath -PathType Leaf)) {
    throw "pgvector manifest not found: $manifestPath"
}

$manifestHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $manifestPath).Hash.ToLowerInvariant()
if ($manifestHash -ne $ExpectedManifestSha256.ToLowerInvariant()) {
    throw "pgvector manifest does not match the approved SHA-256."
}
...
foreach ($artifact in $manifest.artifacts) {
    ...
    $allowed = $relative -eq "lib/vector.dll" -or
        $relative -eq "share/extension/vector.control" -or
        ($relative.StartsWith("share/extension/vector--", [System.StringComparison]::Ordinal) -and
            $relative.EndsWith(".sql", [System.StringComparison]::Ordinal))
    if (-not $allowed) {
        throw "pgvector manifest contains an unapproved installation path: $relative"
    }

    $source = Join-Path $packageRoot $relative
    if (-not (Test-Path -LiteralPath $source -PathType Leaf)) {
        throw "pgvector artifact missing: $source"
    }
    ...
    if ($actual -ne $expected) {
        throw "pgvector artifact failed SHA-256 verification: $relative"
    }
}
