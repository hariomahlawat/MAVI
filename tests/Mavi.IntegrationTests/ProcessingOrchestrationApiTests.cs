...
public sealed class ProcessingOrchestrationApiTests
{
    ...
    public async Task QueueLeaseHeartbeatReclaimFailAndRequeuePreservesHistory()
    {
        ...
        using var leaseA = await client.PostAsJsonAsync("/api/vision/jobs/lease", new LeaseVisionJobRequest("worker-a"));
        ...
        using var heartbeat = await client.PostAsJsonAsync($"/api/vision/jobs/{jobId}/heartbeat",
            new VisionJobHeartbeatRequest("worker-a", 40));
        ...
        using var leaseB = await client.PostAsJsonAsync("/api/vision/jobs/lease", new LeaseVisionJobRequest("worker-b"));
        ...
        using var staleHeartbeat = await client.PostAsJsonAsync($"/api/vision/jobs/{jobId}/heartbeat",
            new VisionJobHeartbeatRequest("worker-a", 50));
        ...
        using var staleFail = await client.PostAsJsonAsync($"/api/vision/jobs/{jobId}/fail",
            new FailVisionJobRequest("worker-a", "stale", null));
        ...
        using var failed = await client.PostAsJsonAsync($"/api/vision/jobs/{jobId}/fail",
            new FailVisionJobRequest("worker-b", "vision_dummy_not_implemented", "bounded diagnostic"));
    ...
    public async Task TwoWorkersRaceForOneQueuedJobAndExactlyOneWins()
    {
        ...
        var calls = new[] {
            clientA.PostAsJsonAsync("/api/vision/jobs/lease", new LeaseVisionJobRequest("worker-a")),
            clientB.PostAsJsonAsync("/api/vision/jobs/lease", new LeaseVisionJobRequest("worker-b")) };
    ...
    public async Task ConcurrentQueueIsProtectedAndAttemptExhaustionTerminalizesChain()
    {
        ...
        {
            using var lease = await a.PostAsJsonAsync("/api/vision/jobs/lease", new LeaseVisionJobRequest($"worker-{attempt}"));
            Assert.Equal(HttpStatusCode.OK, lease.StatusCode);
            clock.Advance(TimeSpan.FromSeconds(120));
        }
        using var exhausted = await a.PostAsJsonAsync("/api/vision/jobs/lease", new LeaseVisionJobRequest("worker-4"));
