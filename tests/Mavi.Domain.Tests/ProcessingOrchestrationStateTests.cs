...
public sealed class ProcessingOrchestrationStateTests
{
    ...
    public void ExactExpiryCanReclaimAndIncrementsAttempt()
    {
        ...
        job.Lease("worker-a", Now, TimeSpan.FromSeconds(10), 2);
        Assert.True(job.CanLease(Now.AddSeconds(10), 2));
        job.Lease("worker-b", Now.AddSeconds(10), TimeSpan.FromSeconds(10), 2);
    ...
    {
        var job = VisionJob.Create(Guid.CreateVersion7(), "pipeline", Now);
        job.Lease("worker-a", Now, TimeSpan.FromSeconds(120), 3);
        job.Heartbeat("worker-a", 60, Now.AddSeconds(30), TimeSpan.FromSeconds(120));
        Assert.Equal(Now.AddSeconds(150), job.LeaseExpiresAtUtc);
        var error = Assert.Throws<DomainValidationException>(() =>
            job.Heartbeat("worker-a", 25, Now.AddSeconds(31), TimeSpan.FromSeconds(120)));
    ...
    {
        var job = VisionJob.Create(Guid.CreateVersion7(), "pipeline", Now);
        job.Lease("worker-a", Now, TimeSpan.FromSeconds(10), 3);
        Assert.Throws<DomainValidationException>(() => job.Heartbeat("worker-a", 1, Now.AddSeconds(10), TimeSpan.FromSeconds(10)));
        Assert.Throws<DomainValidationException>(() => job.Fail("worker-b", "failed", null, Now.AddSeconds(1)));
    }
