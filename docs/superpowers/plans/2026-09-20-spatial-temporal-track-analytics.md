...
| `Running` | Claimed by the analytics host; `AttemptCount` incremented; a per-attempt lease expiry set. | → `Completed`, → `Failed`, → `Queued` on lease expiry with attempts left |
...
1. **Reconcile** every `ReconcileIntervalSeconds` (default 5): for each camera with an active revision, find `ProcessingRun`s with `Status = Completed AND VisibilitySequence IS NOT NULL` whose video belongs to that camera and for which no `SceneAnalysis` exists for (active revision, current algorithm version), and insert `Queued` units. Scope of automatic reconciliation is **runs completed after the revision was activated** plus **runs completed after deployment**; older runs are analysed only on explicit request (§AD).
2. **Execute**: claim one `Queued` unit with `SELECT … FOR UPDATE SKIP LOCKED`, mark `Running` with `AttemptCount + 1` and `LeaseExpiresAtUtc = now + LeaseSeconds` (default 300), commit; run the engine outside any transaction; then in one transaction: delete any facts for this unit id (idempotency), insert facts and outcomes, take the exclusive barrier lock, allocate a `processing_visibility_sequence`, mark `Completed`, mark older `Completed` units for the same run `Superseded`, commit. A unit whose lease expired while `Running` is reclaimed as `Queued` by the reconciler if `AttemptCount < MaximumAttempts` (default 3), otherwise `Failed` with `analytics_attempts_exhausted`.
...
Configuration lives in `SceneAnalyticsOptions` (validated on start like `VisionProcessingOptions`): `Enabled`, `ReconcileIntervalSeconds`, `LeaseSeconds`, `MaximumAttempts`, `MaxConcurrentUnits` (default 1 in Development).
...
| Aggregates | Computed over covered runs only; the same `analyticsCoverage` block is returned; a request whose scope has **zero** covered runs returns `200` with empty series and `coverage.evaluatedRuns = 0`, plus the pending/failed counts, so the UI renders "Not analysed yet", never a zero-valued chart. |
...
analyticsCoverage: {
  ...
  complete: bool   // pendingRuns == failedRuns == staleRuns == 0
}
```

An optional query flag `analyticsCoverage=complete` makes the service return `409 track_analytics_incomplete` with the same block in the problem details for callers that must not work with partial data (bulk exports later). The UI uses the metadata to show a persistent "N runs not yet analysed for this scene" notice with a link to the Processing view; it never renders an empty list without that notice when `complete = false`.
...
| `SceneConfigurationRevision` | `Id` (UUIDv7), `SceneConfigurationId`, `RevisionNumber` (1, 2, …, unique per configuration), `CreatedAtUtc`, `CreatedBy` (string; `development-operator` until identity exists, §AG), `Note` (≤ 500), `ReferenceFrameArtifactId?`, `ReferenceFrameVideoAssetId?`, `ReferenceFrameOffsetMs?` | Immutable after insert. |
...
- **Confirmation (debounce):** the crossing is recorded only if the reference point was on the departing side by at least `ε` at some sample within the previous `k` samples (or since Track start) and reaches at least `ε` on the arriving side within the next `k` samples (or by Track end). Otherwise it is jitter and ignored.
...
- **Direction:** `aToB` if the arriving side is the left side of `A→B` in image coordinates (with `y` down, this is a fixed sign convention documented in code and drawn as an arrow in the editor); else `bToA`. Undirected lines still record direction; the labels are what the operator sees.
...
- A sample is **stationary-candidate** if the maximum displacement of the smoothed point within the trailing window of duration `T_window` is below `d_stat`.
...
| `scene_analyses` | analysis unit | `id`, `processing_run_id`, `revision_id`, `algorithm_version`, `parameters_sha256`, `status`, `attempt_count`, `lease_expires_at_utc`, `queued/started/completed_at_utc`, `visibility_sequence`, `analysed_track_count`, `unavailable_track_count`, `duration_ms`, `failure_code`, `failure_details` | Unique (`processing_run_id`, `revision_id`, `algorithm_version`) |
...
Rules: a query that uses any analytic key is an **analytic query**; the repository joins the fact tables through `scene_analyses` filtered by `revision_id`, `algorithm_version = current`, `status = 'Completed'` and `visibility_sequence ≤ snapshotVisibilitySequence`. The fingerprint includes every new key and the **resolved** `sceneRevisionId` (so a revision activation between pages invalidates the cursor with `track_search_invalid`, exactly as a filter change does). Ordering, keyset pagination, the one-hour cursor age and the default latest-visible-run scope are unchanged. `processingRunId` continues to select a historical run, and an analytic query against it reports that run's coverage.
...
Required measurements before any further index or cache: (1) unit duration and rows written for the development corpus and for a synthetic 1 000-Track run; (2) latency of each §S predicate and each §T aggregate at 10⁵ facts with `EXPLAIN (ANALYZE, BUFFERS)`; (3) heatmap time for 50 runs. Results go in the slice-13 report; indexes beyond §Y are added only against a measured plan.
...
**Decision: .NET Application layer**, module `Mavi.Application/Modules/SceneAnalytics` with the geometry engine as a pure, dependency-free library class set (`Geometry`, `VisitDetector`, `CrossingDetector`, `StationaryDetector`, `LoiteringRule`, `AnalysisEngine`) unit-tested in `Mavi.Application.Tests`; repositories and the hosted service in `Mavi.Infrastructure`; endpoints in `Mavi.Api`. The trajectory msgpack parser is a small in-house decoder mirroring `trajectory.py`/`trajectory.ts` validation (no new package; `MessagePack` NuGet was considered and rejected because the format is three fields).
...
| DB integration (IntegrationTests) | `SceneConfigurationPersistenceTests`, `SceneAnalyticsLifecycleTests` | uniqueness constraints; revision history retention; reconciler queues only eligible runs; claim with `SKIP LOCKED` under two concurrent hosts; lease expiry requeue; attempts exhausted → `Failed`; retry; re-analysis creates a new unit and supersedes; facts deleted only for the same unit id; visibility sequence allocated under the barrier. |
| Search integration (IntegrationTests) | `TrackSearchAnalyticsApiTests` | each §S predicate; coverage metadata for pending/failed/stale runs; `analyticsCoverage=complete` 409; cursor invalidated on revision activation; snapshot excludes a unit completed after the first page; historical `sceneRevisionId`; whitelist rejection of malformed keys. |
...
| UI (Vitest) | `features/scene-editor/*.test.tsx`, `visual-search/*analytics*.test.tsx`, `video-review/*analytics*.test.tsx` | draw/edit/delete/validation/unsaved guard; `unprojectPoint` round-trip with letterboxing; filter canonicalisation for new keys; coverage notice rendering; overlay alignment against fixture geometry; explanation panel; keyboard paths. |
...
**Decision: future runs automatically; existing runs on demand.** The reconciler's automatic scope is runs completed after the camera's active revision was activated (which, for a camera configured after deployment, is every run completed from then on). Existing runs show readiness `NotConfigured` (no scene) or `Stale`/`Pending` only after an explicit "Analyse existing runs" request creates units. No full historical backfill is forced; the operator chooses per camera. Ordinary search over existing Tracks is unaffected.
...
| Host restart mid-unit | (lease expiry) | per unit | yes | Reclaimed by reconciler after `LeaseSeconds` |
| Attempts exhausted | `analytics_attempts_exhausted` | per unit | explicit retry only | `Failed`, visible in readiness |
...
On the unit and in `GET …/analytics`: state, attempts, queued/started/completed times, duration, algorithm version and parameters hash, revision number, analysed and unavailable Track counts, failure code. In the Processing page's run detail: one line "Scene analytics: Ready (revision 4, 212 Tracks, 3 unavailable)" or "Pending / Failed (code) [Retry]" with a link to the scene page; the queue view adds a readiness badge. Structured logs from the hosted service at unit start/completion/failure with unit id, run id, revision, duration. No engineering diagnostics in the operator UI beyond the disclosure in §N.

---

## AG. Security and authorization

No new authentication is introduced. Scene saves and re-analysis requests record `CreatedBy = "development-operator"` (or a caller-supplied display name in Development, clearly labelled as unverified) so that when stage 10 brings Windows-integrated identity the same fields carry a verified principal without a schema change. Mutations that should become attributable then: saving a scene revision, requesting re-analysis, retrying a failed unit. Nothing here blocks on the identity architecture.
...
**New evidence required:** golden-fixture determinism across OSes in CI; the real-stack corpus results on the development laptop (facts and overlays for the three scripted videos through the real worker path); the §Z measurements; the ADR of slice 0. Recorded in a dated review document under `docs/reviews/` at slice 13.

---

## AJ. Implementation slices

Each slice is one PR against `main`, merged before the next starts. "Must not include" guards scope.

| # | Slice | Scope | Likely files/modules | Prerequisites | Tests | Acceptance | Must not include |
|---|---|---|---|---|---|---|---|
| 0 | ADR and contracts | ADR "Scene configuration revisions and the scene-analytics post-processing stage" recording §G, §I, §J, §K–§R decisions; `Mavi.Contracts/Api/Scene*`, `Analytics*` records; JSON examples under `contracts/examples` | `docs/decisions/ADR-<next free number>-scene-analytics.md` (ADR-010 is reserved by the deferred review/cases plan for operator identity), `src/platform/Mavi.Contracts`, `contracts/`, `verify_repo` example count | this plan reviewed | contract round-trip tests | ADR accepted; `verify_repo` green | any persistence or engine code |
| 1 | Geometry engine | Pure library: geometry primitives, visit/crossing/stationary/loitering detectors, `AnalysisEngine`, trajectory msgpack decoder, algorithm version and parameters, golden fixtures | `Mavi.Application/Modules/SceneAnalytics/Engine/*`, `tests/Mavi.Application.Tests/SceneAnalytics/*`, `tests/fixtures/scene-analytics/*` | 0 | §AC pure geometry and temporal layers, fixtures byte-identical on Windows and Linux | all frozen rules pinned by tests | DB, API, UI |
| 2 | Scene configuration | Aggregates, validation, repository, migration, endpoints (`GET/PUT scene`, revisions) | `Mavi.Domain/Scene/*`, `Mavi.Application/Modules/SceneAnalytics/Configuration/*`, `Mavi.Infrastructure/Persistence/{Configurations,Repositories,Migrations}`, `Mavi.Api/Endpoints/SceneEndpoints.cs` | 0 | Domain, DB integration, API contract | save/activate/read/history with all §I codes | analytics units |
| 3 | Scene editor UI | Reference-frame picker, canvas editor, validation, revision history, `unprojectPoint` | `src/web/mavi-web/src/features/scene-editor/*`, `api/scene.ts`, `features/video-review/overlay.ts` (additive) | 2 | Vitest editor suite, letterbox round-trip, a11y | operator can define and save a revision | analysis, search |
| 4 | Analysis lifecycle | `SceneAnalysis` aggregate, outcomes, options, reconciler and executor hosted service, claim/lease/retry, barrier integration, status and retry/re-analyse endpoints, Processing status readiness | `Mavi.Domain/SceneAnalytics/*`, `Mavi.Infrastructure/SceneAnalytics/SceneAnalyticsHostedService.cs`, repositories, migration, `ProcessingEndpoints`, `VideoEndpoints` (readiness field) | 1, 2 | Domain state machine; DB lifecycle suite incl. two-host claim, lease expiry, retry, supersede | units run end-to-end on the fixture harness and write outcomes | fact tables beyond outcomes; search |
| 5 | Facts persistence | Visits, summaries, crossings, motion tables; engine output → rows; idempotent rewrite | migration, repositories | 4 | DB integration for uniqueness and idempotency; fixture facts persisted and re-read equal | facts persisted for the corpus | search, UI |
| 6 | Search integration | `TrackSearchQuery` and whitelist extension, fingerprint, repository joins gated by snapshot, coverage block, item and detail `analytics`, `searchState.ts` keys and filter UI group | `TrackSearchQuery/Service/Repository`, `TrackCursorCodec`, `TrackEndpoints`, contracts, `searchState.ts`, `SearchFilterRail.tsx`, `VisualSearchPage.tsx` | 5 | search integration suite; `searchState` tests; coverage notice UI tests | every §S predicate works with pagination and coverage | aggregates |
| 7 | Evidence overlays and explanation | Zone/line/crossing/interval overlays; analytics panel; unavailable/not-analysed notices | `TrackInspector.tsx`, `VideoReviewPage.tsx`, `TrackEvidencePlayer.tsx`, `TrackDetailsPanels.tsx` | 6 | overlay alignment with fixture geometry; explanation content | operator can verify every match visually | aggregates |
| 8 | Aggregates and heatmap | Aggregate endpoints, heatmap endpoint, camera analytics view | `Mavi.Application/Modules/SceneAnalytics/Aggregates/*`, `Mavi.Api/Endpoints/AnalyticsEndpoints.cs`, `features/scene-analytics/*` | 6 | aggregate integration tests with fixture facts; heatmap unit tests | counts, occupancy, peak, heatmap render with coverage | persisted heatmaps |
| 9 | Hardening and performance | §Z measurements on the corpus and synthetic volumes; index adjustments only against measured plans; cold review; failure-injection tests | as measured | 8 | performance report; P1/P2 fixes | report committed; no P1/P2 | new features |
| 10 | Acceptance, qualification, docs | Real-stack corpus run on the laptop; `docs/reviews/<date>-scene-analytics-acceptance.md`; runbook updates (`local-development.md` scene editor and analytics host); roadmap status update | `docs/` | 9 | all suites; `verify_repo` | §13-style acceptance met and recorded | code changes other than doc-driven fixes |
...
**Decision: ten bounded sequential PRs, each from a short-lived branch off `main`, merged in order 0→10, no stacking.** Branch names `feature/scene-analytics-s<N>-<topic>`. Each PR: exact-head CI green on all triggered workflows; cold review at slices 0, 1, 4, 6 and 9 (the decision-bearing slices) and a light review elsewhere; no PR opens against another PR's branch; if a later slice needs a change to an earlier one, it is a small commit in the later slice's PR, not a reopened earlier PR. Stop gates: a slice does not start until the previous merged and its post-merge `main` CI is green.
