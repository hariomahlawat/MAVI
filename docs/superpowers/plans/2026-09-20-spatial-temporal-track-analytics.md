...
**Status:** Stage 1 active. Slices 0–3 are merged; UI-4 is merged through PR #61; **Slice 4 (Search integration and analytics readiness) is implemented on `feature/scene-analytics-s4-search-readiness` and awaits its independent cold review (PR open, not merged).** This document is frozen for Slice 4 after the 2026-09-21 pre-implementation review; the implementation followed it without a plan change.  
**Base for the Slice 4 freeze:** `main@9dbd74d73f00cafa03fb5f9b250191acd7630565` (PR #61 merge).  
...
**UI Foundation gate (added 2026-09-20, after slice 2 merged; status updated 2026-09-21 after PR #61).** The slice numbering above is unchanged (0–7). The UI Foundation programme **UI-1 → UI-2 → UI-3 → UI-4 → UI-5** defined in §33 of `docs/architecture/ui-ux-design-specification.md` and accepted by ADR-012 is inserted between slice 2 and slice 3. **UI-1, UI-2, UI-3 and UI-4 are merged; PR #61 merged as `9dbd74d7`, and exact-main Task 17 Acceptance #1010 plus Quality Gate #1866 are green. Slice 4 has since been merged as `b505a97`. UI-5 is implemented and awaiting review on `feature/ui-5-evidence-player-review`: one Evidence Player serves Review and the Investigation inspector, with the typed timeline lane seam slice 5 draws into. Slice 5 begins once UI-5 has merged with post-merge `main` green.** (The gate as written was: slice 3 does not begin until UI-2 has merged and post-merge `main` is green — UI-1 alone did not lift it.) Slice 3's remaining follow-on work MAY proceed in parallel with UI-4/UI-5 where it introduces no frontend surface. Slice 4's Ledger readiness/coverage indicators required UI-3, which is now in place; slice 4's Investigation filter and coverage UI requires UI-4; slice 5's evidence-overlay and explanation UI requires UI-5; slice 6's heatmap UI uses the UI-2 Workbench grammar; slice 7's acceptance evidence includes the §26 visual QA standard. These are UI preconditions layered on top of the prerequisites in the table, not replacements for them. UI-1 also leaves two evidence-hue sub-decisions to this plan: the **event-marker hue** closes in slice 5 and the **heatmap scale** in slice 6, each chosen from the frozen `--geo-*`/`--evidence-*` namespace and validated against real evidence when that surface first draws one (§8.3 and §32 sub-decisions 2a and 2c of the UI/UX specification).
