...
**Status:** Stage 1 active. **Slices 0–6 and UI-1 through UI-5 are merged. Slice 7 (hardening, performance, acceptance and Stage-1 closure) is in progress**: PostgreSQL 18 qualification, real-worker Development acceptance, cancellation/failure at volume, concurrency race probes, the C1 explanation/heatmap/UI legs and accessibility/visual QA have passed, and the three scripted videos (`tests/fixtures/scene-analytics/scripted-corpus-v1.json`) passed `check` through the fixture worker and real analytics host while disconnected (runbook actions C and D.2). The aggregate P2 is closed as RETAIN. **Stage-1 functional acceptance is closed: exit-gate items 1–19 are PASS**; item 20, post-merge verification on `main`, follows the merge of PR #70. This is recorded in `docs/reviews/2026-09-22-scene-analytics-stage1-acceptance.md`. It is governed by `docs/superpowers/plans/2026-09-22-scene-analytics-s7-hardening-acceptance.md`. Slice 6 merged in PR #68 as `27179187`, delivering aggregates, occupancy and on-demand heatmap.  
