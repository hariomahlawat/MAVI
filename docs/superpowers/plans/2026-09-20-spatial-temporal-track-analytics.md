...
**Status:** Stage 1 active. **Slices 0–6 and UI-1 through UI-5 are merged. Slice 7 (hardening, performance, acceptance and Stage-1 closure) is in progress**: PostgreSQL 18 qualification, real-worker Development acceptance, cancellation/failure at volume, concurrency race probes, the C1 explanation/heatmap/UI legs and accessibility/visual QA have passed; the three scripted videos of the real-stack corpus exist and are engine-proved (`tests/fixtures/scene-analytics/scripted-corpus-v1.json`) but have not yet run through the real worker. The aggregate P2 is closed as RETAIN. Stage 1 is **not** closed: exit-gate item 14 remains, which needs the scripted videos run through the real worker while disconnected (runbook actions C and D.2, one session). This is recorded in `docs/reviews/2026-09-22-scene-analytics-stage1-acceptance.md`. It is governed by `docs/superpowers/plans/2026-09-22-scene-analytics-s7-hardening-acceptance.md`. Slice 6 merged in PR #68 as `27179187`, delivering aggregates, occupancy and on-demand heatmap.  
...
| Overlays project normalised coordinates into the true video content rectangle; the shared Evidence Player owns the media clock/rAF lifecycle; `TrackEvidence` composes raw box/trajectory evidence through the typed layer contract and one timeline. | `shared/evidence/{EvidencePlayer,useEvidenceTransport,projection,timeline}.ts(x)`, `features/video-review/TrackEvidence.tsx`, `trajectory.ts` | Slice 5 adds zone/line/crossing evidence through these extension seams; it does not create another player or media controller. |
...
**Decision: partial-results metadata, not an error.** Every response to an **analytics-dependent query** (§S) carries:

```
analyticsCoverage: {
  sceneRevisionId, algorithmVersion,
  evaluatedRuns, pendingRuns, failedRuns, notConfiguredRuns, disabledRuns, staleRuns,
  analysedTracks, unavailableTracks,
  complete: bool   // every unevaluated run bucket is zero
}
```

`evaluatedRuns` counts runs whose facts were actually evaluated for the query; every other run in the query's scope falls into exactly one of the five remaining buckets. `notConfiguredRuns` is possible only when the query defaults to the active scene and the camera has never had a revision. `disabledRuns` means the **resolved revision itself** is empty: for the normal/default case that is the camera's empty active revision; for an explicit historical `sceneRevisionId`, the historical revision's own `AnalyticsEnabled` value governs instead of the camera's current state. Thus a camera that is disabled today may still evaluate facts from an explicitly named older enabled revision. The two buckets are never merged: an operator is owed the difference between a gap and a switch-off. `complete` is true only when **every** run in scope was evaluable, which means `pendingRuns`, `failedRuns`, `notConfiguredRuns`, `disabledRuns` and `staleRuns` are all zero; an unconfigured or disabled run was not evaluated for an analytics-dependent query, so it makes `complete = false` like a pending one.

**Coverage denominator (Decision, frozen for Slice 4).** Coverage is computed over the distinct `ProcessingRun`s represented by the **non-analytic base Track query after all existing Track filters and latest-run scoping have been applied, but before analytic predicates and before pagination**. Camera/video/run scope, object class, UTC window, minimum duration and minimum confidence therefore narrow both the candidate Tracks and the coverage denominator. Analytic predicates then evaluate only the covered runs in that already-scoped candidate set. The coverage strip consequently describes the set the operator actually asked about, not unrelated runs of the same camera.

**Coverage is cursor-snapshot stable (Decision, frozen for Slice 4).** The coverage block returned on page 1 is part of the analytic search snapshot and **must remain byte-for-byte equivalent on every continuation page of that cursor chain**, even if a queued/running unit changes lifecycle state while the operator pages. The implementation must pin the first-page coverage semantics rather than recomputing mutable readiness buckets afresh for each continuation. A new search resolves fresh coverage.

`AnalyticsCoverageResponse` also carries `analysedTracks` and `unavailableTracks`. These are evidence-accounting counts **within the same non-analytic base Track candidate set used for the coverage denominator**, not whole-unit totals and not additional run-readiness buckets. `unavailableTracks > 0` does **not** by itself make `complete = false` because the owning analysis unit completed, but the UI must disclose the limitation (for example, "3 Tracks could not be analysed") so a complete run-level coverage claim is never read as complete Track evidence.

An optional query flag `analyticsCoverage=complete` makes the service return `409 track_analytics_incomplete` (the **same structured coverage block** in the problem details) whenever any of the five unevaluated-run buckets is non-zero, for callers that must not work with partial data (bulk exports later). The interactive Search UI uses partial coverage and does not expose `analyticsCoverage` as a normal rail control. The UI shows a persistent coverage strip naming each non-zero bucket ("3 runs not yet analysed, 1 run's camera has analytics disabled") plus unavailable-Track evidence, with a link to the Processing view; it never renders an empty list or a zero-valued aggregate without that strip when `complete = false`.

An analytic predicate that names a zone or line outside the **resolved revision**, or an explicit revision that does not belong to the resolved camera, is `400 track_search_invalid` like any other malformed filter. A zone/line predicate against `NotConfigured` is therefore invalid because no revision exists; a zone/line predicate against an empty active revision is invalid unless the caller explicitly names a historical enabled revision that contains that stable geometry id.
...
**Corrupt, missing or analytically insufficient trajectory.** If the artefact is missing from the evidence root, its SHA-256 does not match `Artifact.Sha256`, msgpack fails the v1 validation, or Scene Analytics v1 has fewer than two samples to derive a path, the Track receives an outcome row with `Outcome = Unavailable` and `Reason` ∈ {`trajectory_missing`, `trajectory_integrity_failed`, `trajectory_invalid`, `trajectory_too_short`} and **no derived analytics facts**. The analysis unit still completes; `UnavailableTrackCount` is visible on the unit and in coverage metadata. A Track is never silently treated as "no events". **This does not invalidate the raw Track evidence:** a one-observation Track has one legitimate persisted trajectory sample and UI-5 continues to render that sample; `trajectory_too_short` means only that Scene Analytics v1 cannot derive path-based zone/crossing/motion facts from it.
...
| Key | Type | Semantics |
|---|---|---|
| `sceneRevisionId` | GUID | Revision to evaluate against; default = the camera's active revision. Valid only when `cameraId`, `videoAssetId` or `processingRunId` resolves one camera. |
| `analyticsAlgorithmVersion` | `scene-analytics-v<major>` | Historical algorithm version to evaluate. **Requires an explicit `sceneRevisionId`** and is intended for reproducibility/deep links, not as a normal rail control. Omitted = current engine version. |
...
**Analytic-query classification and dependency matrix (Decision, frozen for Slice 4).** The predicate keys are `zoneId`, `zoneRelation`, `minDwellMs`, `lineId`, `crossingDirection`, `motionDirection`, `minStationaryMs` and `loitering`. Supplying `sceneRevisionId` also makes the request analytics-dependent because it chooses the fact identity. `analyticsAlgorithmVersion` is valid only with an explicit `sceneRevisionId`; together they choose an exact historical identity. `analyticsCoverage` is a control flag, not a predicate; **either value** (`partial` or `complete`) without another analytics-dependent key is invalid rather than silently turning an ordinary Track query into an analytics query. When another analytics-dependent key is present, explicit `partial` canonicalises to omission because partial is the default.

Dependencies are closed: `zoneRelation` and `minDwellMs` require `zoneId`; `crossingDirection` requires `lineId`; `analyticsAlgorithmVersion` requires `sceneRevisionId`; `loitering=true` may stand alone or combine with `zoneId`; `motionDirection` and `minStationaryMs` stand alone. `motionDirection` accepts only the eight directional values `N|NE|E|SE|S|SW|W|NW` — the engine's `None` heading is evidence, not a useful filter. `loitering` accepts only `true`; false is represented by omission. All supplied analytic predicates combine with **AND semantics**. `zoneRelation` defaults to `dwelled` only when `zoneId` is supplied; an explicit `dwelled` and an omitted relation canonicalise identically. Likewise explicit `analyticsCoverage=partial` canonicalises to omission **only inside an otherwise analytics-dependent query** because partial is the default.

The web canonical state enforces the same graph: removing `zoneId` atomically removes `zoneRelation` and `minDwellMs`; removing `lineId` removes `crossingDirection`; removing `sceneRevisionId` removes `analyticsAlgorithmVersion`. A change to a camera-resolving scope (`cameraId`, `videoAssetId`, `processingRunId`) conservatively clears scene-local identity filters (`sceneRevisionId`, zone/relation/dwell and line/direction) rather than carrying geometry identifiers into another camera. Generic motion/stationary/loitering intent may remain only while some committed scope still resolves a single camera; otherwise all analytics-dependent keys are cleared together. Global `loitering=true` may survive removal of a zone.

For any analytics-dependent query the repository joins the fact tables through `scene_analyses` filtered by `revision_id`, `algorithm_version`, **a fact-bearing status (`Completed` or `Superseded`, §G)** and `visibility_sequence ≤ snapshotVisibilitySequence`.
...
**Analytic predicates require a single-camera scope (Decision).** A zone, a line and a revision only mean anything for one camera, and one coverage block names one revision, so an analytics-dependent query must resolve to exactly one camera through `cameraId`, `videoAssetId` or `processingRunId`. Before analytic execution, every supplied scope identifier is resolved; all supplied identifiers must resolve and must resolve to **the same camera**. Missing, nonexistent or conflicting scope in an analytics-dependent query is `400 track_search_invalid`. Ordinary non-analytic search keeps its existing behaviour. Multi-camera analytic search would need per-camera coverage and pinning and is out of this increment.

**First-page resolution and transaction boundary (Decision, frozen for Slice 4).** Do not resolve the active scene in the controller and then independently search later. The first analytic page is coordinated as one repository operation: after syntactic validation, it opens the same read transaction that acquires the shared processing-visibility barrier, resolves every supplied camera/video/run scope to one camera, reads the explicit or active immutable scene revision, captures the current algorithm version, allocates the visibility snapshot, computes the base candidate scope and coverage, and fetches the first page. It returns a `ResolvedAnalyticsSearchContext` plus rows/coverage to the Application service. The service then fingerprints the canonical client keys **plus that returned resolved identity** and encodes cursor v3. Continuation decodes the pinned identity first and the repository validates it belongs to the supplied scope; it never re-resolves "current" revision/version. This ordering gives one linearisation point and prevents a scene activation between two unrelated reads from changing the first page's meaning.

**Identity is resolved once and followed everywhere (Decision, corrected after Slice 4 cold review).** See ADR-011 Decision 8. Two consequences bind implementations. First, the shared processing-visibility barrier is the **first** statement of the first-page transaction, before any camera, revision or geometry is read, and scene activation takes the exclusive counterpart exactly as run completion does; ordering the search against a party that never locks would only look safe. Second, result-associated surfaces read the revision the response pinned, never the camera's current active revision, while draft and configuration surfaces deliberately keep following current. Where a response has resolved an identity, no client may rebuild one from the request it sent.
...
- **First page:** resolve the effective `sceneRevisionId` (the explicit query value, else the camera's active revision at that instant) and effective `algorithmVersion` (the explicit `analyticsAlgorithmVersion`, else the current engine version); allocate the visibility snapshot as today; compute the filter fingerprint over the canonical supplied keys **plus the resolved revision id and algorithm version**; issue an **analytic cursor v3** carrying snapshot time, snapshot sequence, keyset position, fingerprint, `sceneRevisionId`, `algorithmVersion` and a compact fixed-order coverage snapshot. **v3 is integrity-protected before any embedded coverage or identity is trusted:** the canonical payload is authenticated with HMAC-SHA-256 using a per-installation 32-byte cursor-signing key held in server configuration; decode verifies the MAC in constant time before deserialising/using the payload. The key is generated by setup for a new installation, persisted with the installation's server configuration, never returned by an API, never logged, and is independent of the filter fingerprint (which remains an identity/equality mechanism, not a security mechanism). Rotating/loss of this key intentionally invalidates outstanding v3 cursors; ordinary v2 cursors contain no trusted server-derived analytics state and remain unchanged. No external package is introduced; the implementation uses the platform cryptography/runtime already present. Ordinary searches continue to issue/accept the existing v2 cursor unchanged. v3 stays opaque to clients; its encoded envelope is bounded separately (768 characters maximum) and a worst-case contract test pins the limit so coverage pinning cannot grow without review. The response's `analyticsCoverage` names the pinned pair.
...
- **Only a never-configured camera resolves to no revision.** `NotConfigured` pins `sceneRevisionId = null`. A camera whose active revision intentionally disables analytics still **has an active revision**; the first page pins that empty revision's real id and reports its runs under `disabledRuns`. Continuation remains bound to that disabled revision even if a later enabled revision is activated, so pages cannot change meaning mid-snapshot.
- **A new search** (no cursor) after activation resolves the new active revision.
- **A cursor is genuinely invalid** only when: it fails to decode; its fingerprint differs from the fingerprint recomputed from the canonical supplied keys plus the *pinned* pair (the client changed filters); it is older than the one-hour limit or ahead of the clock; or it names a revision id that does not belong to the query's camera. Revisions are immutable and never deleted, so a pinned revision cannot disappear. The codec accepts v2 ordinary cursors for their existing one-hour lifetime and v3 analytic cursors; Slice 4 does not invalidate an in-flight ordinary cursor merely because analytics shipped.
...
**Historical identity semantics.** An explicit `analyticsAlgorithmVersion` is accepted only with an explicit `sceneRevisionId`. For any explicitly resolved historical identity, an exact fact-bearing unit is evaluated, an exact Queued/Running unit is pending, and an exact Failed unit is failed. If the resolved scene revision is **not the camera's active revision**, or the requested algorithm version is **not the current engine version**, and no exact unit exists for a candidate run, that run is reported as `staleRuns` rather than `pendingRuns`: the reconciler and the current camera re-analysis endpoint create work only for the active revision/current engine and will not silently manufacture an inactive historical identity. `Pending` for a missing unit is therefore reserved for the active revision/current engine identity, where automatic reconciliation can legitimately create the unit. This keeps historical deep links reproducible without promising work that can never arrive.

**Response contracts (Decision, frozen for Slice 4).** `TrackSearchResponse` gains optional `analyticsCoverage` (present only for analytics-dependent queries). The coverage contract is exactly §H, including `analysedTracks` and `unavailableTracks`.

Each search item gains a bounded optional `analytics` explanation for the **pinned search identity**, not an unbounded fact dump: `sceneRevisionId`, `algorithmVersion`; zero or more matched-zone summaries (`zoneId`, `visitCount`, `totalDwellMs`, `loitering`); zero or more matched-line summaries (`lineId`, `crossingCount`, optional `matchedDirection`); and an optional motion summary (`heading`, `longestStationaryMs`). Search rows do not carry the full visit/crossing history. If a predicate needs a crossing time for operator explanation, the row may carry only the first matching crossing time; the complete crossing list belongs to detail/Slice 5.

`TrackDetailResponse` analytics are identity-aware: the caller may request an explicit `sceneRevisionId` and `analyticsAlgorithmVersion`, and Search/Review navigation preserves the identity pinned by the originating analytic search. A direct Track link with no analytic identity may resolve the camera's current active revision/current engine. The detail response returns the full facts for the requested/resolved identity plus compact summaries of other analysed identities; it must **never silently substitute the current active revision** when the Track was opened from a historical or superseded analytic search.

The detail analytics payload mirrors persisted fact families rather than inventing a generic event bag: outcome/reason/reference point; zone summaries and visits; line crossings; and the motion summary including stationary intervals. It carries the producing scene revision number/id and algorithm version. Internal analysis ids are not operator-facing identifiers. Slice 4 exposes these typed facts through the API and inspector summary only; Slice 5 owns geometry drawing, timeline lanes and seek-to-evidence interaction.

**Slice 4 operator presentation (Decision).** The Investigation rail exposes an **Analytics** group only on the UI-4 grammar. Zone and line choices are resolved from the selected camera/revision; metadata failure never drops or broadens committed ids, and unresolved ids remain visible as shortened identifiers. `minDwellMs` and `minStationaryMs` remain milliseconds in the URL/API but are edited as seconds in the rail with the same exact decimal-preservation discipline used by the existing duration/confidence fields. Screen headings are rendered as **Up, Up-right, Right, Down-right, Down, Down-left, Left, Up-left** while the wire remains `N|NE|E|SE|S|SW|W|NW`; these are image directions, never compass bearings. `analyticsAlgorithmVersion` is visible as a committed chip when present but has no normal rail selector in Slice 4.

The coverage strip sits immediately beneath the results header/chips and persists for the search snapshot. Ordinary zero results remain the existing zero-results state. An analytics-dependent query with zero matches and incomplete coverage says that no **evaluated** Tracks matched and names the unevaluated buckets; it must not reuse the ordinary zero-results message. `unavailableTracks` is disclosed separately from run readiness.

**Processing readiness presentation (Decision).** Processing Queue keeps its existing single video-processing status badge. Analytics readiness appears in a separate compact Analytics column as text/non-colour state treatment — **never a second status badge**. Processing Detail may show the richer lifecycle status from `GET /api/processing/runs/{runId}/analytics`: Ready names revision/version and analysed/unavailable Track counts; Failed exposes **Retry analytics**; Stale explains that current scene geometry/engine is not applied and offers the existing camera-level re-analysis action with copy that clearly states its camera-wide `latestRuns` consequence; Disabled and NotConfigured remain distinct. Queue and Detail continue polling while `analyticsReadiness == Pending` even after video processing is already Processed, and stop analytics polling in Ready/Failed/Stale/Disabled/NotConfigured. Mutations invalidate both processing-readiness and run-analytics queries. No new per-run re-analysis endpoint is invented in Slice 4.
...
Slice 5 is rebaselined in the implementation plan `docs/superpowers/plans/2026-09-22-scene-analytics-s5-evidence-explanation.md`, written against the merged UI-5 foundation rather than the deleted pre-UI-5 player.

Frozen Slice-5 presentation decisions:

- `TrackEvidence` remains the Track adapter over the single shared `EvidencePlayer`; no legacy/private player or second media controller is introduced.
- The exact `TrackDetailAnalytics.sceneRevisionId/sceneRevisionNumber/algorithmVersion` identity is authoritative. Full geometry is loaded from that immutable revision and verified against the expected revision id; the active revision is never a historical-evidence fallback.
- Zone and trip-line overlays reuse the shared evidence projection. Geometry with persisted Track facts receives emphasis without changing its underlying evidence role; disabled geometry keeps the disabled spatial grammar.
- Crossing points are rendered from persisted `TrackDetailLineCrossing.pointX/pointY`; the browser never recomputes a crossing from the trajectory.
- The one timeline closes UI decision 7 with **bounded stacked analytical lanes**: subject, zone/dwell and stationary. Dwell/stationary use hatched patterns, not new hues. Crossings and real zone boundary entry/exit are markers.
...
| Search integration (IntegrationTests) | `TrackSearchAnalyticsApiTests` | ordinary non-analytic search and v2-cursor regression parity; every §S predicate and every invalid dependency/canonical-default combination; analytics-control-only rejection; single-camera resolution through camera/video/run and rejection of missing, nonexistent or conflicting scopes; zone/line ownership against the resolved revision; coverage denominator after existing Track filters; pending, failed, stale, not-configured and disabled buckets separately, including a currently-disabled camera querying an explicit older enabled revision; analysed/unavailable Track accounting within the base candidate set; `complete` only when all five run buckets are zero; `analyticsCoverage=complete` 409 with the same structured coverage block; **revision-pinned v3 cursor:** page 1 resolves revision 4, revision 5 is activated, page 2 still evaluates revision 4 and returns identical pinned coverage, and a new search resolves revision 5; disabled-camera first page pins the actual empty revision id while never-configured pins null; cursor rejected only for changed filters, age, skew, decode failure or a foreign revision id; tampered v3 payload/signature is rejected before embedded coverage is trusted; setup/configuration provides a stable per-install cursor-signing key and key rotation invalidates v3 only; worst-case analytic cursor remains within the 768-character envelope; snapshot excludes a unit completed after the first page; explicit historical `sceneRevisionId`; historical `analyticsAlgorithmVersion` requires that revision and absent old-version units classify stale, not pending; **a pinned identity whose unit is `Superseded` still returns its facts**, both for an explicit historical query and for a continuation page whose unit was superseded after page 1, while readiness for the active revision remains `Stale` rather than `Ready`; whitelist rejection of malformed keys. |
...
| 3 (done) | Analytics lifecycle and derived facts | `SceneAnalysis` aggregate with fenced attempt ownership (`RequireOwnership`), outcome rows, options, reconciler and executor hosted service, claim/lease/reclaim/retry, visibility-sequence integration, idempotent fact writes for zone visits, zone summaries, line crossings and motion summaries, Processing readiness, status and retry/re-analysis endpoints | `Mavi.Domain/SceneAnalytics/*`, `Mavi.Infrastructure/SceneAnalytics/SceneAnalyticsHostedService.cs`, repositories, migration, `ProcessingEndpoints`, `VideoEndpoints` (readiness field) | 1 | Domain state machine incl. ownership fencing and the explicit-retry cycle reset; DB lifecycle suite incl. two-host claim, lease expiry and reclaim, expired-but-not-reclaimed completion, the seven-step stale-completion scenario and stale failure rejection, retry, re-analysis idempotency per identity under repeat and concurrency, supersede; fact uniqueness and idempotent rewrite | units run end to end on the fixture harness and persist facts and outcomes; a stale attempt cannot alter a reclaimed unit; disabled and unconfigured cameras are never queued | search, UI, aggregates |
| 4 (done) | Search integration and analytics readiness | `TrackSearchQuery` and strict whitelist/dependency extension; resolved single-camera analytics context; HMAC-authenticated v3 analytic cursor with revision/algorithm/coverage pinning while v2 ordinary cursors remain valid; snapshot-gated repository joins; `analyticsCoverage` with run buckets and unavailable-Track accounting; bounded item analytics and typed identity-aware detail analytics; `searchState.ts` keys/dependency clearing; Investigation Analytics filter group, committed chips and persistent coverage strip; Processing Queue and Processing Detail readiness using the Slice-3 lifecycle/status endpoints, including analytics polling/retry/re-analysis semantics | `TrackSearchQuery/Service/Repository`, `TrackCursorCodec`, cursor-signing options/setup configuration, `TrackEndpoints`, contracts, `api/tracks.ts`, `api/sceneAnalytics.ts`, `api/videos.ts`, `searchState.ts`, `SearchFilterRail.tsx`, `CommittedFilterChips.tsx`, `VisualSearchPage.tsx`, `ProcessingQueuePage.tsx`, `ProcessingPage.tsx` | 3 + UI-3/UI-4 merged | search integration suite incl. v2 ordinary-cursor compatibility and revision/algorithm/coverage-pinned v3 continuation; `searchState` dependency/canonicalisation tests; coverage-strip/zero-result UI tests; six readiness states and polling transitions on Processing Queue/Detail | every §S predicate works with pagination; coverage is snapshot-stable and `complete` only when all five unevaluated-run buckets are zero; unavailable Tracks are disclosed; incomplete analytics never render as ordinary zero matches; Processing surfaces distinguish Ready/Pending/Failed/Stale/Disabled/NotConfigured without a duplicate status badge and Pending progresses without manual refresh | evidence overlays, aggregates |
| 5 (done) | Evidence overlays and explanation | Exact-revision zone/line overlays, persisted crossing points, exact-seek crossing/entry/exit markers, bounded stacked zone/dwell + stationary lanes, matched/context geometry treatment, shared explanation, revision/engine/reference-point disclosure, and distinct unavailable/not-analysed states | `TrackEvidence.tsx`, `shared/evidence/{EvidenceTimeline,timeline}.tsx/ts`, analytical evidence adapter/explanation components, `TrackInspector.tsx`, `VideoReviewPage.tsx`, scene-revision retrieval, tests/visual-QA fixtures | 4 + UI-5 merged | pure evidence-model fixtures; pinned-revision fail-closed tests; marker exact-seek keyboard/pointer tests; letterbox/pillarbox overlay tests; explanation/state tests; Review/Investigation regressions | an operator can visually verify every persisted analytic fact against the exact revision that produced it, seek to exact crossings, and distinguish raw evidence from unavailable analytics | aggregates, heatmaps, Stage-7 events, trajectory v2, live |
...
| 7 | Hardening, performance, acceptance, qualification and docs closure | §Z measurements on the corpus and synthetic volumes; index review against real `EXPLAIN` plans; failure injection; full real-stack deterministic corpus on the laptop; browser trajectory-parser parity hardening (reject normalized x/y outside [0,1], matching worker/application decoder); independent cold review and P1/P2 remediation; analytics qualification evidence; runbook and roadmap updates; final exact-head validation | `docs/reviews/<date>-scene-analytics-acceptance.md`, `docs/runbooks/local-development.md`, roadmap status, indexes as measured | 2, 5, 6 | all suites; `verify_repo`; performance report | the §Acceptance criteria met and recorded; no P1/P2; measurements committed | any new capability |

**UI Foundation gate (status rebaselined 2026-09-22 after PR #68).** The slice numbering remains 0–7. **UI-1 through UI-5 and Scene Analytics Slices 0–6 are merged.** UI-5 / PR #64 established one shared Evidence Player; Slice 5 / PR #66 consumed that seam and closed bounded analytical-lane decision 7 and crossing-colour decision 2a; Slice 6 / PR #68 consumed the UI-2 Workbench grammar and closed heatmap scale decision 2c against its rendered surface. **Slice 7 is the remaining Stage-1 closure unit and includes §26 visual QA.**

Trajectory v2 (bottom-centre reference point) is deliberately **not** a Stage-1 slice. It remains a separately qualified future worker-side improvement and does not alter the authoritative capability-roadmap sequence; after Stage-1 closure, the current roadmap advances to Stage 2 — Visual Attributes unless that roadmap is explicitly amended.
