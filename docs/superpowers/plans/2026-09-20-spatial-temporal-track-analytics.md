...
**Historical algorithm-version semantics.** An explicit `analyticsAlgorithmVersion` is accepted only with an explicit `sceneRevisionId`. For the requested pair, an exact fact-bearing unit is evaluated, an exact Queued/Running unit is pending, and an exact Failed unit is failed. If the requested version is not the current engine version and no exact unit exists for a candidate run, that run is reported as `staleRuns` rather than `pendingRuns`: the installed engine will not silently manufacture a historical version. This keeps historical deep links reproducible without implying that old-version work is waiting to run.
