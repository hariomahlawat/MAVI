...
| Search integration (IntegrationTests) | `TrackSearchAnalyticsApiTests` | ordinary non-analytic search regression parity; every §S predicate and every invalid dependency combination; analytics-control-only rejection; single-camera resolution through camera/video/run and rejection of missing, nonexistent or conflicting scopes; coverage denominator after existing Track filters; pending, failed, stale, not-configured and disabled buckets separately; unavailable-Track accounting; `complete` only when all five run buckets are zero; `analyticsCoverage=complete` 409 with the same structured coverage block; **revision-pinned cursor:** page 1 resolves revision 4, revision 5 is activated, page 2 still evaluates revision 4 and returns identical pinned coverage, and a new search resolves revision 5; disabled-camera first page pins the actual empty revision id while never-configured pins null; cursor rejected only for changed filters, age, skew, decode failure or a foreign revision id; snapshot excludes a unit completed after the first page; explicit historical `sceneRevisionId`; explicit historical `analyticsAlgorithmVersion`; **a pinned identity whose unit is `Superseded` still returns its facts**, both for an explicit historical query and for a continuation page whose unit was superseded after page 1, while readiness for the active revision remains `Stale` rather than `Ready`; whitelist rejection of malformed keys. |
