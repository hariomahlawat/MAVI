...
| `analyticsAlgorithmVersion` | string | Algorithm version to evaluate; default = the current engine version. Exact historical versions remain queryable. |
...
**Analytic-query classification and dependency matrix (Decision, frozen for Slice 4).** The predicate keys are `zoneId`, `zoneRelation`, `minDwellMs`, `lineId`, `crossingDirection`, `motionDirection`, `minStationaryMs` and `loitering`. Supplying `sceneRevisionId` or `analyticsAlgorithmVersion` also makes the request analytics-dependent because it chooses the fact identity. `analyticsCoverage` is a control flag, not a predicate; `analyticsCoverage=complete` **without another analytics-dependent key is invalid** rather than silently turning an ordinary Track query into an analytics query.

Dependencies are closed: `zoneRelation` and `minDwellMs` require `zoneId`; `crossingDirection` requires `lineId`; `loitering=true` may stand alone or combine with `zoneId`; `motionDirection` and `minStationaryMs` stand alone. All supplied analytic predicates combine with **AND semantics**. `zoneRelation` defaults to `dwelled` only when `zoneId` is supplied. The web canonical state enforces the same graph: removing `zoneId` atomically removes `zoneRelation` and `minDwellMs`; removing `lineId` removes `crossingDirection`; changing camera or scene revision clears zone/line ids that do not exist in the newly resolved revision. Global `loitering=true` may survive removal of a zone.
...
- **First page:** resolve the effective `sceneRevisionId` (the explicit query value, else the camera's active revision at that instant) and effective `algorithmVersion` (the explicit `analyticsAlgorithmVersion`, else the current engine version); allocate the visibility snapshot as today; compute the filter fingerprint over the client-supplied keys **plus the resolved revision id and algorithm version**; issue a cursor whose payload (a new cursor version) carries snapshot time, snapshot sequence, keyset position, fingerprint, `sceneRevisionId`, `algorithmVersion` and the pinned coverage state required by §H. The response's `analyticsCoverage` names the pinned pair.
...
| 4 | Search integration and analytics readiness | `TrackSearchQuery` and strict whitelist/dependency extension; resolved single-camera analytics context; revision- and algorithm-pinned fingerprint/cursor plus pinned coverage; snapshot-gated repository joins; `analyticsCoverage` with run buckets and unavailable-Track accounting; bounded item analytics and identity-aware detail analytics; `searchState.ts` keys/dependency clearing; Investigation Analytics filter group, committed chips and persistent coverage strip; Processing Queue and Processing Detail readiness using the Slice-3 lifecycle/status endpoints | `TrackSearchQuery/Service/Repository`, `TrackCursorCodec`, `TrackEndpoints`, contracts, `api/tracks.ts`, `api/sceneAnalytics.ts`, `api/videos.ts`, `searchState.ts`, `SearchFilterRail.tsx`, `CommittedFilterChips.tsx`, `VisualSearchPage.tsx`, `ProcessingQueuePage.tsx`, `ProcessingPage.tsx` | 3 + UI-3/UI-4 merged | search integration suite incl. revision/algorithm/coverage-pinned continuation; `searchState` dependency/canonicalisation tests; coverage-strip/zero-result UI tests; six readiness states on Processing Queue/Detail | every §S predicate works with pagination; coverage is snapshot-stable and `complete` only when all five unevaluated-run buckets are zero; unavailable Tracks are disclosed; incomplete analytics never render as ordinary zero matches; Processing surfaces distinguish Ready/Pending/Failed/Stale/Disabled/NotConfigured without a duplicate status badge | evidence overlays, aggregates |
