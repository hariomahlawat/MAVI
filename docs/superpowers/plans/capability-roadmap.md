...
| Visual attributes | **Next — architecture planning active** | `VisualAttribute` groundwork exists. Stage-2 draft establishes a bounded Track Evidence Set plus modular post-Track intelligence lifecycle so models can be replaced/re-run independently; see `2026-09-23-visual-attributes.md` and proposed ADR-013. | Stage 2 |
...
**Visual Attributes (stage 2).** Active draft: `2026-09-23-visual-attributes.md`. Attributes are evidence-backed, immutable observations produced by a replaceable post-Track intelligence component over a bounded capability-neutral Track Evidence Set. Initial scope is person upper/lower clothing colour, bag/backpack, qualified headwear/helmet, and vehicle dominant colour. Vehicle subclass remains Stage 3 unless deliberately rebaselined. No attribute is exposed unless its frozen qualification gate is met.
...
4. **Visual Attributes — Stage 2, NEXT.** Architecture-planning branch active. Freeze `2026-09-23-visual-attributes.md`, proposed ADR-013 and the predeclared qualification protocol through independent review before implementation.
