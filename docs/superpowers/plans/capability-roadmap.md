...
3. Spatial & Temporal Track Analytics: Slice 0 (ADR and contracts) then the bounded slices in its plan; stage dependencies and impacts per `capability-implementation-roadmap.md`. Slices 0–3 are done; Slice 3 (analytics lifecycle and derived facts) merged through PR #59.
   ...
   **UI Foundation programme — inserted after Scene Analytics Slice 2, before Slice 3.** Strictly sequential: **UI-1 → UI-2 → UI-3 → UI-4 → UI-5**, defined in §33 of `docs/architecture/ui-ux-design-specification.md`. **UI-1 (design foundation) and UI-2 (shell and workspace grammar) are merged, with post-merge `main` green**, which lifted the Slice 3 gate; **Scene Analytics Slice 3 has since been completed and merged (PR #59)**. **UI-3 (existing operational surfaces) is merged (PR #60) with post-merge `main` green; UI-4 (Investigation workspace) is the active UI Foundation increment.** Backend/domain analytics work MAY proceed in parallel with UI-4/UI-5 where it introduces no frontend surface; new user-facing analytics UI lands only on the established workspace grammar — **UI-3 is in place, so Ledger readiness/coverage indicators (Slice 4) are no longer UI-gated; UI-4 is still required before Slice 4 Search/readiness UI, and UI-5 before Slice 5 evidence-overlay UI**, and Slice 6 heatmap UI uses the UI-2 Workbench grammar. This programme is UI sequencing; it does not renumber Scene Analytics slices (0–7) or capability stages.
