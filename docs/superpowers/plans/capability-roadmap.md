...
| Visual attributes | **In progress — slice S1 (Track Evidence Set) under implementation** | Stage-2 architecture is defined by the parent plan plus accepted ADR-013 (Track Evidence / analysis lifecycle) and ADR-014 (capability binding v2). Architecture gate A1–A12 is closed. S1.1 (whole-Track retirement, PR #75) and S1.2a (platform completion v3 + staging janitor, PR #77) are merged; S1.2b (worker trajectory spool, PR #78) is in review; S1.2c (worker Evidence Set + v3 emission) is next. No Stage-2 acceptance row is claimed yet (evidence is S1.4). | Stage 2 |
...
4. **Visual Attributes — Stage 2, IN PROGRESS.** Architecture freeze is complete. Slice S1 is being implemented per `2026-09-23-stage2-s1-2-evidence-set-implementation.md`: S1.1 (PR #75) and S1.2a (PR #77) are merged, S1.2b (trajectory spool, PR #78) is in review, and S1.2c (Evidence Set + v3 emission) follows. S1.3 (read contract and viewer) and S1.4 (qualification evidence) come after.
