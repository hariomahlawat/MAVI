...
| Visual attributes | **In progress — slice S1 (Track Evidence Set) under implementation** | Stage-2 architecture is defined by the parent plan plus accepted ADR-013 (Track Evidence / analysis lifecycle) and ADR-014 (capability binding v2). Architecture gate A1–A12 is closed. S1.1 (whole-Track retirement, PR #75), S1.2a (platform completion v3 + staging janitor, PR #77) and S1.2b (worker trajectory spool, PR #78) are merged; S1.2c (worker Evidence Set + v3 emission) is implemented and in review. The two-tier Representative (ADR-013 §4 amendment) was accepted on 2026-09-24. The real-clip parameter measurement is complete (MOT17-02/13; all selector defaults retained under the current scorer). Merge waits on the owner's decision on finding F1 (the occlusion proxy counts sub-floor detections; parameter note §7). No Stage-2 acceptance row is claimed yet (evidence is S1.4). | Stage 2 |
...
4. **Visual Attributes — Stage 2, IN PROGRESS.** Architecture freeze is complete. Slice S1 is being implemented per `2026-09-23-stage2-s1-2-evidence-set-implementation.md`: S1.1 (PR #75), S1.2a (PR #77) and S1.2b (trajectory spool, PR #78) are merged. S1.2c (Evidence Set + v3 emission) is implemented and in review; the two-tier Representative (ADR-013 §4) was accepted on 2026-09-24, the real-clip parameter measurement is complete with all defaults retained under the current scorer, and merge waits on the owner's decision on its finding F1 (occlusion proxy). S1.3 (read contract and viewer) and S1.4 (qualification evidence) come after.
