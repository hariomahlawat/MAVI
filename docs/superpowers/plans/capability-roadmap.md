...
4. **Visual Attributes — Stage 2, IN PROGRESS.** Architecture freeze is complete. Slice S1 is being implemented per `2026-09-23-stage2-s1-2-evidence-set-implementation.md`: S1.1 (PR #75), S1.2a (PR #77) and S1.2b (trajectory spool, PR #78) are merged. S1.2c (Evidence Set + v3 emission) is implemented and in review; the two-tier Representative (ADR-013 §4) was accepted on 2026-09-24, the real-clip parameter measurement is complete with all defaults retained, and merge waits on the owner's decision on its finding F1 (occlusion proxy). S1.3 (read contract and viewer) and S1.4 (qualification evidence) come after.
