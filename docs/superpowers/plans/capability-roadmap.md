...
| Spatial & temporal Track analytics | **Slice 7 in progress — Stage 1 NOT closed** | Slices 0–6 are merged. Slice 7 closed the browser trajectory-parser range defect and built fail-closed qualification harnesses. On the Development machine, PostgreSQL 18 qualification **PASSED** (every §S/§T case at 110,000 facts, the 1,000-Track unit, and the 50-run/2,000-Track heatmap envelope) at reachable SHA `5f516662…`, and real-worker acceptance and restart persistence **PASSED**. Since then, cancellation/failure at realistic volume, deterministic concurrency race probes, the C1 explanation/heatmap/UI legs and accessibility/visual QA at four widths have **passed**, and the three FFmpeg-scripted videos are built and engine-proved. Still open against the Slice-7 plan §25 gate: the aggregate-materialisation P2 (blocked on transcribing the authoritative §T figures and on approval of its decision rule) and Development-machine operator actions — Development-corpus unit record, the scripted videos through the real worker, runtime identities, the disconnected no-fetch run and the Search → Investigation leg (`docs/runbooks/scene-analytics-stage1-development-acceptance.md`). Status and evidence: `docs/reviews/2026-09-22-scene-analytics-stage1-acceptance.md`. | Stage 1 — parent plan + `2026-09-22-scene-analytics-s7-hardening-acceptance.md` |
...
3. Spatial & Temporal Track Analytics: **Slices 0–6 are merged; Slice 7 is in progress and Stage 1 is NOT closed**, governed by `2026-09-22-scene-analytics-s7-hardening-acceptance.md`. PostgreSQL 18 qualification, real-worker Development acceptance, and the resilience, concurrency, C1 and accessibility/visual-QA items have passed; the remaining §25 items — one open P2 and Development-machine operator actions — are recorded in `docs/reviews/2026-09-22-scene-analytics-stage1-acceptance.md`. No new Stage-1 capability is added in Slice 7. Stage 2 — Visual Attributes must not start until the Stage-1 exit gate is satisfied.
