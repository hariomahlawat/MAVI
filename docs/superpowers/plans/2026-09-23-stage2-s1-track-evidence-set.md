...
**Status:** Implementation-grade parent plan; independently reviewed 2026-09-23. S1.1 and S1.2 are merged. The S1.3 plan is accepted (PR #81); S1.3a (read contract) is implemented in PR #82, in review; S1.3b (UI) and S1.4 are outstanding.  
**Date:** 2026-09-23  
...
**Current implementation state (2026-09-24):** S1.1 merged in PR #75. S1.2a/#77, S1.2b/#78 and S1.2c/#79 are merged; S1.2 is complete on `main@2060599a9786651f36071742034076369520d0ce`. The dedicated S1.3 implementation plan is `docs/superpowers/plans/2026-09-24-stage2-s1-3-evidence-read-ui-implementation.md` (accepted, PR #81). S1.3a, the Track-detail `observations[]` read contract, is implemented in PR #82 and in review (plan §20). S1.3b, the minimal Evidence Set UI, is outstanding.
...
> **Implementation plan:** S1.2 is specified to implementation grade in `docs/superpowers/plans/2026-09-23-stage2-s1-2-evidence-set-implementation.md` (revision 2, baseline `main@4b6141f`, after S1.1). That plan splits S1.2 into **three** independently deployable PRs — S1.2a platform (dual-accept v2/v3, persistence, `GET /api/vision/contract`, and a platform-owned **staging janitor** with database authority), S1.2b worker **trajectory spool** (no wire change; makes per-live-Track memory constant in Track duration), S1.2c worker Evidence Set + v3 emission — and records nine reconciliations with this document in its §17. Three need owner ratification before S1.2c starts: **C1** (the S1-14 Representative fallback reservoir is replaced by the equivalent online "best admissible candidate" rule, with proof and the adversarial encoder measurement retained), **C3** (reclaiming worker staging after a successful completion becomes a platform responsibility — an ADR-006 amendment plus an ADR-013 §5 wording change — because nothing removes a completed attempt's staging today and a worker-side cleanup is not crash-safe), and the parent §6.4 "trajectory-in-progress" live-memory term, which the spool makes a constant (C4). The §16 two-PR split below is superseded by that plan's three-PR split (C8).
