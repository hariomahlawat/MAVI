...
**Status:** Implementation-ready plan, revision 3 (after the second independent cold review; janitor precision only). **S1.2a implemented** (platform v3 and staging janitor; deviations D1–D3 recorded in its PR). **S1.2b implemented** (worker trajectory spool; implementation notes and deviations E1–E7 in §16.1). **S1.2c implemented, not merged** (worker Evidence Set and completion 3.0; deviations E8–E21 and test mapping in §16.2). E8 (two-tier Representative) was **accepted by the owner on 2026-09-24** (ADR-013 §4 amendment). The §5 measurement note is **complete**: the scripted corpus plus MOT17-02-FRCNN and MOT17-13-FRCNN at 1920×1080 through the production RTMDet → ByteTrack → Evidence Set path, with every selector default retained under the current scorer (E23; role-parameter retention provisional). It found **F1**: the occlusion proxy counts sub-floor detections, so 59 % of real Representatives are fallbacks. F1 is a scorer (ADR-013 §4) question, not a parameter, and merge waits on the owner's decision on it.  
**Date:** 2026-09-23  
...
| E23 | §5: real-clip measurement before merge | Measured with `tools/vision/dev/measure_evidence_real_clips.py` on a runtime built from the qualified `linux-x86_64-cpu` definition (supervisor READY, exact Python identity, the hash-locked graph with `mmcv` rebuilt locally from its pinned commit), for MOT17-02-FRCNN and MOT17-13-FRCNN. The official WebM previews are 960×540, so the primary clips are the benchmark's 1920×1080 `img1` JPEGs stream-copied into MP4 (MJPEG, byte-identical packets), and the previews are measured as a supplementary variant. Every default is retained. Finding **F1**: 93 % of occlusion rejections come from detections below `confidenceFloor`, the fallback-Representative rate is 58.7 % and vehicles qualify in 1 of 2,801 frames (parameter note §7) | The official previews are not the benchmark resolution. F1 cannot be fixed by any selector parameter; a fix is a scorer version and an ADR-013 §4 clarification, and that is an owner decision |
...
|---|---|---|
| Selector defaults poorly tuned | measurement note done (scripted + 2 real clips; all defaults retained); `confidenceFloor ≤ activation` enforced | profile revision after S1.4 evidence |
...
| A later Representative replacement makes the NearView holder a near-duplicate; `resolve()` drops it, and a new NearView must beat the dropped holder's area by × (1 + growth) (S1.2c cold review) | deterministic; accepted by §4.2 note 3; measured on real clips: 1 of 42 held NearViews dropped (2.4 %) | measure NearView loss at volume in S1.4 |
| F1: the occlusion proxy counts every detection down to `detectorInferenceFloor` 0.05, including same-object duplicates, so on real crowded scenes 59 % of Representatives are fallbacks and vehicles almost never qualify (parameter note §7) | output stays conservative: supplemental roles qualified-only, E8 fallback keeps every job | owner decision: fix now (scorer `quality-v2`: occluders ≥ `confidenceFloor`; ADR-013 §4 clarification; profile rebind; re-measure) or defer to S1.4 as a recorded residual. Option (a) re-runs the measurement and re-adjudicates every parameter |
