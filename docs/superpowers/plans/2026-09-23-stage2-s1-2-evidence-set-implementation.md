...
| E23 | §5: real-clip measurement before merge | Harness `tools/vision/dev/measure_evidence_real_clips.py` drives the production composition with the READY qualified runtime; a non-interference test runs in the Task 10 qualified job. The measurement itself is **not run**: the MOT17-02/13 sources and the qualified runtime sources are denied by this session's egress policy (parameter note §6) | The gate stays open until the measurement runs on a host that has the qualified runtime |
...
| A later Representative replacement makes the NearView holder a near-duplicate; `resolve()` drops it, and a new NearView must beat the dropped holder's area by × (1 + growth) (S1.2c cold review) | deterministic; accepted by §4.2 note 3 | measure NearView loss in S1.4 |
