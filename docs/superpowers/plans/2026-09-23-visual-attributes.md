...
While a Track is live its current best candidate per role is held encoded (≤544 KiB per live Track). S1 evolves the model-neutral Tracker boundary to emit retired MAVI Track ids exactly once when the adapter guarantees they cannot reappear (ByteTrack: after lost-buffer expiry); VideoProcessor must not duplicate backend timeout logic. On retirement, candidates are written once to attempt-scoped staging and only descriptors/metadata are retained; still-live Tracks are drained at end-of-stream. Worker memory is therefore bounded by live Tracks. Transient staging before admission is bounded by 10,000 × 544 KiB ≈ 5.2 GiB and is cleaned with the attempt. Retirement/no-reappearance/reactivation/end-of-stream semantics are contract-tested before this bound is claimed.
