...
While a Track is live its current best candidate per role is held encoded (≤544 KiB per live Track). S1 evolves the model-neutral Tracker boundary to emit retired MAVI Track ids exactly once when the adapter guarantees they cannot reappear (ByteTrack: after lost-buffer expiry); VideoProcessor must not duplicate backend timeout logic. On retirement the whole Track is finalised — trajectory artefact, Representative and supplemental candidates are staged once and only descriptors plus scalar summary are retained (the trajectory list, not the crops, is the larger resident structure); the adapter releases the native→MAVI id mapping so a MAVI id can never be reused within an attempt; still-live Tracks are drained at end-of-stream. Worker memory is therefore bounded by live Tracks. Staging left by earlier fenced-out attempts of the same job is removed when a new attempt is leased. Transient staging before admission is bounded by 10,000 × 544 KiB ≈ 5.2 GiB and is cleaned with the attempt. Retirement/no-reappearance/reactivation/end-of-stream semantics are contract-tested before this bound is claimed.
...
| **S1 Track Evidence Set** | in-loop selector/encoding; model-neutral Tracker retirement signal with exact-once/no-reappearance semantics; stage accepted candidates once at Track retirement/end-of-stream; completion schema v3 + digest v3 (re-derived body bound), validator/store/sealing, Observation evolution, observations[] and evidence viewer; Task-10/E2E rebinding | Acceptance B1–B6 PASS |
