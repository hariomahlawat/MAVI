...
- Protocol switch (§15.2): `WorkerContractRules.CompletionSchemaVersions` is now `AsynchronousCompletionSchemaVersions` = `["2.0","3.1"]`; `IsAcceptedCompletionSchemaVersion` accepts 2.0 and 3.1. `GET /api/vision/contract` advertises `["2.0","3.1"]`. A 3.0 `POST …/complete` is refused with `worker_contract_version_unsupported` before any store is reached and is never reinterpreted (§15.4). `ProcessingResultStore` (synchronous) now serves 2.0 only; its v3 sealing/graph code is retained, unreachable, for the F3 finalizer to lift.
