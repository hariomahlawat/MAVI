...
**Companion:** `2026-09-20-spatial-temporal-track-analytics.md` is the implementation-grade plan for stage 1. `docs/architecture/ui-ux-design-specification.md` is the adopted UI/UX specification; its §33 UI Foundation programme (UI-1 → UI-5) is inserted after stage-1 slice 2 and gated slice 3 on UI-2. UI-1 and UI-2 are merged, that gate is lifted, slice 3 is merged (PR #59), and **UI-3 is the active increment**. Task 18 (`2026-09-18-task-18-phase1-production-qualification-rebaseline.md`) is a separate, parallel qualification stream and is not a stage here.
...
| Stage 1 slice 3 ← UI Foundation UI-2 | Hard | **Satisfied.** The UI Foundation programme (UI-1 → UI-5, §33 of `docs/architecture/ui-ux-design-specification.md`, accepted by ADR-012) is inserted after stage-1 slice 2. UI-2 merged with green post-merge `main`, which lifted the gate, and slice 3 has since merged. Backend/domain work now runs in parallel with UI-3/UI-4/UI-5 where it introduces no frontend surface. |
| Stage 1 slice 4 ← UI Foundation UI-3 and UI-4 | Hard | Slice 4's Ledger readiness/coverage indicators require **UI-3** (the active increment), and its Investigation Search/readiness UI requires **UI-4**. Slice 4's backend and contract work is not gated. |
...
- **UI foundation.** All stage-1 UI from slice 4 onward lands on the workspace archetypes established by the UI Foundation programme: UI-3 (active) before Ledger readiness/coverage indicators, UI-4 before Investigation analytics UI, UI-5 before slice-5 evidence overlays, and the UI-2 Workbench grammar (merged) for slice-6 aggregates and heatmap.
...
- **Acceptance / non-goals / slices / exit gate.** See the stage-1 plan. Exit gate: acceptance criteria met on the development laptop with the real worker path; analytics-specific evidence recorded; no P1/P2 open; exact-head CI green; the scene-configuration and analytics-lifecycle ADR accepted.
