...
| Stage 1 slice 4 ← UI Foundation UI-3 and UI-4 | Hard | Slice 4's Ledger readiness/coverage indicators required **UI-3**, which is merged, and its Investigation Search/readiness UI requires **UI-4** (the active increment). Slice 4's backend and contract work is not gated. |
...
- **UI foundation.** All stage-1 UI from slice 4 onward lands on the workspace archetypes established by the UI Foundation programme: UI-3 (merged) before Ledger readiness/coverage indicators, UI-4 (active) before Investigation analytics UI, UI-5 before slice-5 evidence overlays, and the UI-2 Workbench grammar (merged) for slice-6 aggregates and heatmap.
