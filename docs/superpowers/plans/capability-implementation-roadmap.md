...
- **UI foundation.** All stage-1 UI from slice 4 onward lands on the workspace archetypes established by the UI Foundation programme: UI-3 before Ledger readiness/coverage indicators, UI-4 before Investigation analytics UI, UI-5 before slice-5 evidence overlays, and the UI-2 Workbench grammar for slice-6 aggregates and heatmap.
