...
| **7** | Timeline lane presentation for multiple analytical interval types (stacked lanes versus single lane with glyphs) | UI-5 defines the extension point; **the rebaselined Slice-5 plan selects bounded stacked analytical lanes (subject + zone/dwell + stationary), but this row closes only when the rendered Slice-5 implementation is validated and merged** | §18; `2026-09-22-scene-analytics-s5-evidence-explanation.md`. |
