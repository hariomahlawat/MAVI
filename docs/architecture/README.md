...
- `docs/decisions/ADR-013-modular-post-track-intelligence-and-evidence.md` — **proposed architecture-freeze candidate** for Stage 2: VisionJob-owned bounded Track Evidence Set, independent attribute lifecycle/process, lease-scoped hash-verified evidence reads, immutable outcomes and v4 search identity.
- `docs/decisions/ADR-014-capability-binding-v2.md` — **proposed architecture-freeze candidate** generalising Runtime/Model Pack binding from the detector-era single-model shape to capability bindings, capability-neutral model manifests and capability-scoped qualification.
