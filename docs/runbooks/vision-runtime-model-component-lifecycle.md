...
3. **Worker emitting 3.0 (S1.2c).** Profile `1.1` (`profileVersion` 1.2.0-candidate). Before its first lease, and again after any control-plane error, the worker calls `GET /api/vision/contract`. A platform that does not list `"3.0"` (404/405, a malformed answer, or a list without it) keeps the worker **not ready**: it logs `vision_platform_contract_unsupported` once, **never leases**, never falls back to 2.0, and re-probes every poll interval, so it becomes ready by itself once the platform is upgraded. A `400 worker_contract_version_unsupported` at completion fails that attempt with `vision_worker_contract_unsupported` and is not retried by the worker.
   ...
   - **Failure codes.** An accepted Track always has a Representative: before the Track's first qualified frame, the best admissible frame holds the role (proposed ADR-013 §4 amendment). If no crop at all can be encoded within its cap, the attempt fails (`vision_processing_failed`) and its staging is removed.
