...
Two of these are load-bearing rather than tuning knobs:
...
- **`MaxConcurrentUnits` defaults to 1.** The host shares the API process, and analytics must not compete with serving an operator's request.

`ReconcileLookbackDays` bounds how far back automatic reconciliation reaches for runs that completed before the host started. Older runs, and runs that predate a geometry edit, are analysed only on explicit re-analysis — a scene edit never silently reinterprets history.
...
The component overlay now declares a `windows-x86_64-cuda` Runtime Pack (C5), and the runtime profile records that variant as `qualified-development-hardware` (C4). The two `Auto` decisions are therefore no longer equivalent, and the difference is deliberate until the CUDA release lock is promoted:

- the **launcher** decides from the installed pack, its declared identity and the driver. On a Development host with the C3 pack installed and the GPU present it selects CUDA (`cuda_selected`) and starts Python with an explicit `cuda` policy; without the pack it records `cuda_pack_absent`;
- the **supervisor** additionally requires the variant's entry in `runtime.json` `releaseLocks` to be `qualified-offline-lock`. That entry is still `pending-hardware-qualification`, so any entry point that reaches Python with `MAVI_DEVICE_POLICY=auto` resolves to CPU and records `cuda_pack_not_declared` (the closed vocabulary's catch-all for a not-ready CUDA runtime; it under-describes the cause and is left as is).

Neither path selects a device silently: the reason is logged, persisted in provenance and returned in the completion contract.
