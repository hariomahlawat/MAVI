...
| **PostgreSQL 18 used for authoritative qualification** | **18.6**, pgvector **0.8.6**, Windows Development machine, canonical service `MAVI-Dev-PostgreSQL-18` (`127.0.0.1:55433`), database `mavi_test`; clean tree at reachable SHA `5f5166628b0c4af04cc8f7efcd40f7022f5095c4` |
...
| **C1 semantic reference** | **PASS** — trajectory → facts → §S → §T, and the explanation, heatmap and UI projection legs | Semantic acceptance §5; Stage-1 register item 2 |
...
**Encoding.** The videos are H.264 **High** profile at CRF 1 with no B-frames. The first version used CRF 0, which x264 can only encode as High 4:4:4 Predictive. Browser decoders commonly refuse that profile, so Evidence Review could not reliably play it. `verify` passes all 300 frames of all three videos at the new setting and still reports 250 problems for a mismatched scenario. Videos generated with the earlier setting must be regenerated before import.
