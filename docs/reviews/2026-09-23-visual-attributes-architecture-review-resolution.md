...
| R-21 | P2 | Analysis identity included runtime-pack variant and build/commit, so a CPU→CUDA host change or any deployment would create a "newer" analysis that supersedes a semantically identical one and mark history Stale — contrary to ADR-011 D3. Found on the cold re-read of the amended head. | ADR-013 §11 (first amended text) | ADR-013 §11/§16 and plan §12/§16: identity is run + schema + pipeline + aggregation + model packs + parameters; variant/device/build are provenance in the digest; variant equivalence becomes a qualification gate (§10). |
| R-22 | P3 | Staging on every candidate replacement would re-write staging many times per Track. | `process_video.py` replacement frequency | ADR-013 §5 and plan §8.3: hold the current best encoded candidate per role in memory while the Track is live; write once at Track end. |
