...
The current Phase-1 product implementation is complete through Task 17. Task 18 remains the active production-qualification and closure stream. The operator application provides Overview, Cameras, Videos, Import, Processing, Search and Review workflows. Current capability development is **capability-first**: after the Windows CUDA stream (PR #49), the next feature is Spatial & Temporal Track Analytics, followed by Visual Attributes, expanded operational object classes, ANPR/OCR, visual similarity/ReID and event analytics. Audited review/cases are intentionally deferred until the system has richer intelligence to investigate; live RTSP/VMS ingestion remains later. See `docs/superpowers/plans/2026-09-20-capability-roadmap-reconciliation.md` for the authoritative capability sequence and `docs/superpowers/plans/2026-09-19-visual-intelligence-workspace.md` for the current search/evidence baseline. ADR-008 defines profile-based qualification: Development remains a single Windows workstation/laptop with Auto/CUDA/CPU device intent, while Production profiles are qualified independently. Implementation completion is distinct from release verification: unavailable profile/device/offline/quality/performance evidence remains explicitly pending, and no Production profile is advertised as supported until its exact evidence passes.
