...
The current Phase-1 product implementation is complete through Task 17. Task 18 remains the active production-qualification and closure stream. The operator application provides Overview, Cameras, Videos, Import, Processing, Search and Review workflows. Current capability development is **capability-first**: with the Windows CUDA Development stream merged (PR #49, Development hardware evidence through C4; C5.4/C6/C7 host execution outstanding), the next feature is Spatial & Temporal Track Analytics, followed by Visual Attributes, expanded operational object classes, ANPR/OCR, visual similarity/ReID and event analytics. Audited review/cases are intentionally deferred until the system has richer intelligence to investigate; live RTSP/VMS ingestion remains a later platform phase. Two roadmap documents are authoritative: `docs/superpowers/plans/capability-roadmap.md` states *what* is built next and in which sequence, and `docs/superpowers/plans/capability-implementation-roadmap.md` states *how* the stages depend on one another technically, what each changes, and what each must not invalidate; `docs/superpowers/plans/2026-09-19-visual-intelligence-workspace.md` records the delivered search/evidence workspace. `docs/architecture/ui-ux-design-specification.md` is the adopted UI/UX design specification and is normative for frontend work; its UI Foundation programme (UI-1 → UI-5) is sequenced after Scene Analytics Slice 2. UI-1 and UI-2 are merged with green post-merge `main`, which lifted the Scene Analytics Slice 3 gate; Slice 3 (analytics lifecycle and derived facts) is merged, UI-3 (existing operational surfaces) is merged with green post-merge `main`, and **UI-4 (Investigation workspace) is the active UI Foundation increment**. UI-4 remains required before Slice 4 Search/readiness UI, and UI-5 before Slice 5 evidence-overlay UI. ADR-008 defines profile-based qualification: Development remains a single Windows workstation/laptop with Auto/CUDA/CPU device intent, while Production profiles are qualified independently. Implementation completion is distinct from release verification: unavailable profile/device/offline/quality/performance evidence remains explicitly pending, and no Production profile is advertised as supported until its exact evidence passes.
